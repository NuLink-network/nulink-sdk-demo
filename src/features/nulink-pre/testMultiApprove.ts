  ...
  */
  ...
  for (let index = 0; index < aliceApprovedfilesList['list'].length; index++) {
    ...
    if (element['file_id'] === applyFileId) {
      ...
      const bobBeApprovedfilesList = await getApprovedFilesAsUser(accountBob, 1, 1000)
      /*return data format: {
                              list: [
                                { apply_id, file_id:, proposer, proposer_id, file_owner:, file_owner_id:, policy_id, hrac, start_at:, end_at, created_at }
                                ...
      ...
      */
      ...
      const arrayBuffer: ArrayBuffer = await getFileContentByFileIdAsUser(accountBob, bobBeApprovedfilesInfo['file_id'])
